...
curl -i -X POST ${HEC_IAM_ENDPOINT}/v3/auth/tokens -H 'content-type: application/json' -d "$AUTH_PARAMS" > /tmp/hec_auth_res && {
    ...
    PRIVATEIPS_ID='195702d9-870b-42d7-bf95-8bd7a2826fd7'
    UPT_PRIVATEIP_PARAMS='{
        "privateips": {
            {
                "subnet_id": "e959b4f0-6c5e-403c-a28f-3aaf71771e29"
            }
        }
    }'
    curl -i -X GET https://vpc.cn-north-1.myhwclouds.com/v1/${PROJECT_ID}/privateips/$PRIVATEIPS_ID -H 'content-type: application/json' -H "X-Auth-Token:${TOKEN}" -k
}
