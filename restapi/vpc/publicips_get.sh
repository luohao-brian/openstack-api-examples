...
curl -i -X POST ${HEC_IAM_ENDPOINT}/v3/auth/tokens -H 'content-type: application/json' -d "$AUTH_PARAMS" > /tmp/hec_auth_res && {
    ...
    PUBLIC_ID='30c77073-947a-45aa-b4ac-fc8ba44cd594'
    
    curl -i -X GET https://vpc.cn-north-1.myhwclouds.com/v1/${PROJECT_ID}/publicips/$PUBLIC_ID -H 'content-type: application/json' -H "X-Auth-Token:${TOKEN}" -k
}
