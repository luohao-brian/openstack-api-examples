...
curl -i -X POST ${HEC_IAM_ENDPOINT}/v3/auth/tokens -H 'content-type: application/json' -d "$AUTH_PARAMS" > /tmp/hec_auth_res && {
    ...
    BANDWIDTHS_ID='df15f64b-01f1-4370-8563-d71688095386'
    UPT_BAND_PARAMS='{
        "bandwidth": {
            "name": "bandwidth-upt",
            "size": "90"
    ...
    curl -i -X PUT https://vpc.cn-north-1.myhwclouds.com/v1/${PROJECT_ID}/bandwidths/$BANDWIDTHS_ID -H 'content-type: application/json' -H "X-Auth-Token:${TOKEN}" -d "$UPT_BAND_PARAMS" -k
}
