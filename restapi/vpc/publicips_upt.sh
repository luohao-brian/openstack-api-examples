...
curl -i -X POST ${HEC_IAM_ENDPOINT}/v3/auth/tokens -H 'content-type: application/json' -d "$AUTH_PARAMS" > /tmp/hec_auth_res && {
    ...
    UPT_PUBLICIPS_PARAMS='{
        "publicip": {
            "port_id": "195702d9-870b-42d7-bf95-8bd7a2826fd7"
        }
    }'
    PUBLICIP_ID='30c77073-947a-45aa-b4ac-fc8ba44cd594'
    curl -i -X PUT https://vpc.cn-north-1.myhwclouds.com/v1/${PROJECT_ID}/publicips/$PUBLICIP_ID -H 'content-type: application/json' -H "X-Auth-Token:${TOKEN}" -d "$UPT_PUBLICIPS_PARAMS" -k
}
