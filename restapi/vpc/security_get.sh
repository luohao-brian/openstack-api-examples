...
curl -i -X POST ${HEC_IAM_ENDPOINT}/v3/auth/tokens -H 'content-type: application/json' -d "$AUTH_PARAMS" > /tmp/hec_auth_res && {
    ...
    curl -i -X GET https://vpc.cn-north-1.myhwclouds.com/v1/${PROJECT_ID}/security-groups/$SECURITY_ID -H 'content-type: application/json' -H "X-Auth-Token:${TOKEN}" -d "$CREATE_SECURITY_PARAMS" -k
}
