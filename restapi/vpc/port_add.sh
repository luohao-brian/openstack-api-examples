...
curl -i -X POST ${HEC_IAM_ENDPOINT}/v3/auth/tokens -H 'content-type: application/json' -d "$AUTH_PARAMS" > /tmp/hec_auth_res && {
    ...
    CREATE_SUBNET_PARAMS='{
        "port": {
            "name": "port-gtx",
            "admin_state_up": true,
            "network_id":"e959b4f0-6c5e-403c-a28f-3aaf71771e29"
            }
    }'

    curl -i -X POST https://vpc.cn-north-1.myhwclouds.com/v1/ports -H 'content-type: application/json' -H "X-Auth-Token:${TOKEN}" -d "$CREATE_SUBNET_PARAMS" -k
}
