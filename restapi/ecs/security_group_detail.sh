...
SECURITY_GROUP_ID='1d598a85-4697-4cfb-b086-2629068dde06'
...
curl -i -X POST ${HEC_IAM_ENDPOINT}/v3/auth/tokens -H 'content-type: application/json' -d "$AUTH_PARAMS" > /tmp/hec_auth_res && {
    ...
    curl -s -X GET ${HEC_ECS_ENDPOINT}/v2/${PROJECT_ID}/os-security-groups/${SECURITY_GROUP_ID} -H "X-Auth-Token:${TOKEN}" | python -mjson.tool
}
