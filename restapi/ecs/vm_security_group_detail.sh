...
SERVER_ID='d2d0a91e-8cf2-43cd-8ebb-cac3789d199f'
...
curl -i -X POST ${HEC_IAM_ENDPOINT}/v3/auth/tokens -H 'content-type: application/json' -d "$AUTH_PARAMS" > /tmp/hec_auth_res && {
    ...
    curl -s -X GET ${HEC_ECS_ENDPOINT}/v2/${PROJECT_ID}/servers/${SERVER_ID}/os-security-groups -H "X-Auth-Token:${TOKEN}" | python -mjson.tool
}
