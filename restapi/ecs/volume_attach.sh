...
curl -i -X POST ${HEC_IAM_ENDPOINT}/v3/auth/tokens -H 'content-type: application/json' -d "$AUTH_PARAMS" > /tmp/hec_auth_res && {
    ...
    "volumeAttachment": {
        "volumeId": "6dd9b9c0-8b16-4e2b-b7c7-0f380927a3c9",
        "device": "/dev/sdb"
    }
  }'
   
    curl -i -X POST https://ecs.cn-north-1.myhwclouds.com/v1/${PROJECT_ID}/cloudservers/${SERVER_ID}/attachvolume -H "X-Auth-Token:${TOKEN}" -d "$ATTACH_VOLUME_PARAMS" 
}
