...
curl -i -X POST ${HEC_IAM_ENDPOINT}/v3/auth/tokens -H 'content-type: application/json' -d "$AUTH_PARAMS" > /tmp/hec_auth_res && {
    ...
    curl -i -X POST ${HEC_ECS_ENDPOINT}/v2/${PROJECT_ID}/servers/${SERVER_ID}/action -H 'content-type: application/json' -H "X-Auth-Token:${TOKEN}" -d "${FLOATING_RELEASE_BODY}"
}
