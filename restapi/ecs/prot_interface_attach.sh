...
curl -i -X POST ${HEC_IAM_ENDPOINT}/v3/auth/tokens -H 'content-type: application/json' -d "$AUTH_PARAMS" > /tmp/hec_auth_res && {
    ...
    ATTACH_PORT_PARAMS='{
    "interfaceAttachment": {
        "port_id": "70249530-f64c-44d2-862f-68b2e6caa2d3",
    }
  }'

    curl -i -X POST https://ecs.cn-north-1.myhwclouds.com/v2/${PROJECT_ID}/servers/${SERVER_ID}/os-interface -H "X-Auth-Token:${TOKEN}" -d "$ATTACH_PORT_PARAMS"
}
